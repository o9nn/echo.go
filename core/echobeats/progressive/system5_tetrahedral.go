...
type TetrahedralState struct {
	Streams    [4]Sys5StreamState
	Universals [3]Sys5UniversalState
	Timestamp  time.Time
	StepIndex  int
}

// StreamState represents the state of a tetrahedral stream.
type Sys5StreamState struct {
	ID       int
	State    int
	Thought  *consciousness.LLMThought
}

// Sys5UniversalState represents the state of a universal rotator in System 5.
type Sys5UniversalState struct {
	ID    int
	State string
}
...
func (s *System5Tetrahedral) transitionStream(ctx context.Context, streamIdx int, universalIdx int) error {
	...
	_, err := stream.thoughtEngine.GenerateAutonomousThought(ctx, thoughtType)
	if err != nil {
		// Fallback thought on error - logged but not stored
		_ = &consciousness.LLMThought{
...
func (s *System5Tetrahedral) captureState() TetrahedralState {
	...
	for i := 0; i < 4; i++ {
		...
		state.Streams[i] = Sys5StreamState{
			ID:    s.streams[i].id,
	...
	for i := 0; i < 3; i++ {
		...
		state.Universals[i] = Sys5UniversalState{
