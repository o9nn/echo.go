	...
	for _, match := range matches {
		...
		if gfxOverride == "" {
			...
			if !slices.Contains[[]string, string](supported, gfx) {
				...
				slog.Warn("See https://github.com/cogpy/echo9llama/blob/main/docs/gpu.md#overrides for HSA_OVERRIDE_GFX_VERSION usage")
				continue
			} else {
...
func AMDValidateLibDir() (string, error) {
	...
	slog.Warn("amdgpu detected, but no compatible rocm library found.  Either install rocm v6, or follow manual install instructions at https://github.com/cogpy/echo9llama/blob/main/docs/linux.md#manual-install")
