...
// GPU memory constants moved to gpu_constants.go
...
var (
	...
	// unsupportedGPUs and bootstrapErrors moved to gpu_constants.go
)

// GPU compute requirements and limits moved to gpu_constants.go
	...
	for _, libPath := range nvcudaLibPaths {
		...
		if resp.err != nil {
			...
			case C.CUDA_ERROR_UNKNOWN:
				err = fmt.Errorf("unknown error initializing cuda driver library %s: %s. see https://github.com/o9nn/echo.go/blob/main/docs/troubleshooting.md for more information", libPath, C.GoString(resp.err))
				slog.Warn(err.Error())
			default:
